...
void renderFrameData(FrameData dt) {
  ...
  image(
    p1Img,
    dt.widthCenter - (2 * FrameData.ledPanelWidth * FrameData.imgDpSize),
    dt.heightCenter
  );
  ...
  textFont(h2Font);
  renderStringRow(
    "TOP", centerX, 120, color(255)
  );
  renderStringRow(
    padZeros(dt.topScore, 7),
    centerX, 160, dt.arcadeWhite
  );
  ...
  renderStringRow(
    "1UP", leftX, 160, dt.arcadeCyan
  );
  renderStringRow(
    padZeros(dt.p1Score, 7),
    leftX, 200, dt.arcadeWhite
  );
  renderStringRow(
    dt.p1Name.substring(0, Math.min(dt.p1Name.length(), 8)),
    leftX, 240, dt.arcadeOrange
  );
  renderStringRow(
    "2UP", rightX, 160, dt.arcadeCyan
  );
  renderStringRow(
    padZeros(dt.p2Score, 7),
    rightX, 200, dt.arcadeWhite
  );
  renderStringRow(
    dt.p2Name.substring(0, Math.min(dt.p2Name.length(), 8)),
    rightX, 240, dt.arcadeOrange
  );
}
